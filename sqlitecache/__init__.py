...
class SQLiteCache(object):
    ...
    def dumpToFile(self):
        if 'y' == input('Write db dump to memory?'):
            f = open('dump.db','w')
        ...
        if len(removeList)>0 or len(addList)>0:
            if 'y' == input("Changes detected ({} added, {} removed), perform rescan? (y/n)".format(len(addList),len(removeList))):
                ...
                for removeItem in removeList:
                    print('removing'+removeItem +' ...'+self.eta(starttime,removed/len(removeList)))
    ...
    def fetchFileIds(self,terms):
        fileids = []
        query = '''SELECT frowid FROM search LEFT JOIN dictionary ON search.drowid = dictionary.rowid WHERE '''
        where = ' or '.join([' dictionary.word = ?']*len(terms))
        limit = ' LIMIT 0, 150'
        self.db.execute(query+where+limit,tuple(terms));
